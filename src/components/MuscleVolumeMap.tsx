...
export interface MuscleVolumeMapProps {
  ...
  mapSize?: number;
  /**
   * `dropdown` es el selector por defecto: con doce grupos, el carril
   * horizontal obligaba a arrastrar mucho para llegar a los últimos.
   */
  selectorLayout?: 'dropdown' | 'rail' | 'grid';
...
export function MuscleVolumeMap({
  ...
  const handlePress = ({ muscles }: MuscleMapPressEvent) => {
    ...
    // Elegir desde el cuerpo equivale a elegir en la rejilla: se cierra para
    // dejar el desglose inmediatamente visible.
        ...
        <View style={styles.explore}>
          ...
          {selectorLayout === 'dropdown' ? (
            <MuscleDropdown
              ...
              onSelect={(muscle) => setSelectedMuscles([muscle])}
            />
          ) : selectorLayout === 'rail' ? (
            <ScrollView
              horizontal
...
interface MuscleDropdownProps {
  ...
  onSelect: (muscle: VolumeMuscleGroup) => void;
}

/**
 * Disparador compacto + rejilla desplegable. En reposo ocupa una fila y muestra
 * la selección vigente (venga del mapa o de la propia rejilla); al abrirse
 * expone los doce grupos a la vez, sin desplazamiento lateral.
 */
function MuscleDropdown({
  ...
  const selectedResults = results.filter((result) =>
    selectedMuscles.includes(result.muscle),
  );
  const leadingZone = selectedResults.reduce<MuscleVolumeZone>(
    (zone, result) =>
      ZONE_PRIORITY[result.zone] > ZONE_PRIORITY[zone] ? result.zone : zone,
    'none',
  );
  const totalSets = selectedResults.reduce(
    (sum, result) => sum + result.totalSets,
    0,
  );
  ...
  const summary =
    selectedResults.length === 0
      ...
      : selectedResults.length === 1
        ? MUSCLE_GROUP_LABELS[selectedResults[0].muscle]
        : `${selectedResults.length} músculos`;
  const detail =
    selectedResults.length === 0
      ? `${results.length} con volumen`
      : `${formatEquivalentSets(totalSets)} series`;

  const handleSelect = (muscle: VolumeMuscleGroup) => {
    onSelect(muscle);
    onOpenChange(false);
  };
  ...
  return (
    <View style={styles.dropdown}>
      <PressableScale
        ...
        accessibilityLabel={`Músculo: ${summary}`}
        accessibilityHint={
          open ? 'Cierra la lista de músculos' : 'Abre la lista de músculos'
        }
      ...
      >
        <View
          style={[styles.zoneDot, { backgroundColor: ZONE_COLORS[leadingZone] }]}
        />
        <Text weight="semibold" numberOfLines={1} style={styles.dropdownSummary}>
          {summary}
        </Text>
        ...
        >
          {results.map((result) => (
            <MuscleSelector
              ...
              selected={selectedMuscles.includes(result.muscle)}
              layout="grid"
              onPress={() => handleSelect(result.muscle)}
            />
          ))}
        </Animated.View>
      ) : null}
