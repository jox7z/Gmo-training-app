...
Target Expo SDK 54, React Native 0.81, React 19, Expo Router, and **Expo Go**.
Use native primitives; do not translate browser CSS patterns literally.
...
- Use the nearly-square radius scale (`radius.xs` 2 … `radius.xl` 4).
  Use `radius.pill` for variable-width capsules and reserve `radius.full` for
  real circles.
...
- Prefer open space and typographic hierarchy over nested borders. Ordinary
  cards/buttons do not receive decorative glow. Keep labels in sentence case,
  titles short, values cream and metadata muted. Reserve red for actions, records
  and relevant states.
...
- Progress uses a fixed 6×7 month calendar and a searchable 12-muscle milestone
  selector; cells keep a 4-point GitHub-style gap. The body map is an optional
  shortcut, not the only selection path.
...
- Feed pull-to-refresh keeps content fixed; only the transparent GMO indicator
  moves. Its header contains the short `Social` title, Search and Notifications;
  do not restore greeting copy or a dedicated refresh button.
