...
**Update your agent memory** as you discover recurring patterns, file structure conventions, key module locations, common symbol naming patterns, and codebase-specific search shortcuts. This builds up institutional knowledge across conversations.

Examples of what to record:
- Entry points and barrel files (e.g., `src/index.ts` re-exports everything from `src/features/`)
- Naming conventions (e.g., all services are in `src/services/*.service.ts`)
- Monorepo package boundaries and which packages contain which domains
- Frequently searched symbols and their canonical locations
- Language/framework patterns used (e.g., project uses path aliases like `@app/` mapped to `src/`)
