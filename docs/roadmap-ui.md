...
| **Gmo hoy** | Tracking set-by-set interactivo con RestRing animado (ventana de recuperación verde), SetProgressPills, duración/descanso por serie, PRs detectados y comparación con sesión previa en el modal de resultados. **Falta:** rest timer muere en background y no notifica; sin "anterior" por fila ni autofill; RPE en schema sin UI. Supersets ✅ MVP 2026-07-24 (pares de 2; ver B2 de roadmap.md). Trisets/circuitos (2–4 miembros) + warm-ups dentro de un grupo ✅ 2026-07-24 (sprint 8). |
...
| Score de optimización visible mientras se edita la rutina (patrón Fitbod: feedback antes de guardar) | P1 | S | — | D3 · ✅ 2026-07-27 (sprint 9) — el card del score (círculo + barras de breakdown + grupos débiles) se extrajo de `app/(tabs)/routines.tsx` a `src/components/routines/RoutineScoreCard.tsx` (presentación pura, prop `score: RoutineScore` + `style`) y se montó también en el editor `app/routine/[id].tsx`, bajo Visibilidad/aiReasoning y antes de los tabs de días. `useMemo(() => computeRoutineScore(routine, profile))` sobre el BORRADOR en memoria: recalcula con cada cambio de ejercicios/sets sin guardar primero. 100% local, sin query/RPC/migración nueva (`computeRoutineScore` ya existía y solo lee `profile.weeklyGoalDays` + el catálogo estático de ejercicios). Revisión de `code-quality-reviewer` encontró 1 hallazgo real: el gate de "rutina vacía" usaba `weakGroups.length > 0`, que por construcción de `computeRoutineScore` (siempre 2 elementos) nunca es falso — mostraba "grupos más débiles" fabricados en una rutina sin ejercicios; corregido a `breakdown.coverage > 0` antes de commitear. |
| Leaderboard con alcance Mi Rango / Global en Progreso (antes solo comparaba contra tu mismo rango) | P1 | S | — | D2 · ✅ 2026-07-27 (sprint 9) — `SegmentedControl` nuevo en la sección "Leaderboard" de `app/(tabs)/progress.tsx`: Mi rango sigue con `useLeaderboard(rankId)` (sin cambios), Global agrega `useGlobalLeaderboard(50, enabled)` — mismo hook que ya usaba el tab "Ranking" de `app/discover.tsx`, con `enabled` nuevo para no dispararlo hasta elegir ese modo (mismo `queryKey`, cache compartida entre las dos pantallas). Las dos filas de usuario casi idénticas que existían por separado (`RankingRow` de discover, `LeaderboardRow` local de progress) se consolidaron en `src/components/social/LeaderboardRow.tsx` (`variant: 'card'|'list'`, navegable solo si se pasa `onPress`), de paso corrigiendo dos inconsistencias con la convención de `PressableScale`/`SkeletonRow` que ninguna de las dos filas originales seguía. Sin migración: RLS de `profiles` ya es lectura pública. |
| Frases motivacionales animadas durante la serie (no solo en descanso) | P2 | S | — | D5 · ✅ 2026-07-27 — `MotivationalPhrase` (generalizado de `RestPhrase`, misma animación `Animated`: entrada spring overshoot + slide-up, salida fade/shrink, ciclo de 4.5s) + array nuevo `SET_MOTIVATION_PHRASES` (tono de empuje, distinto al de recuperación de `REST_PHRASES`), montado en `SetPhase` bajo el chip del cronómetro (`app/workout/active.tsx`). 100% local: sin dependencias, sin estado persistido, sin Supabase. |
