-- Enriquece posts de workout usando solo series efectivas completadas y
-- plausibles. `posts.metadata` ya es jsonb: no requiere columnas nuevas.
-- Conserva claves legacy para que publicaciones y clientes anteriores sigan
-- siendo legibles.
...
begin
  ...
  select w.user_id, w.is_published, w.duration_seconds, w.started_at, w.ended_at
    into v_owner, v_published, v_stored_duration, v_started, v_ended
  ...
  -- PRs conservan el contrato previo, ahora con los mismos límites plausibles.
    ...
    with workout_bests as (
      select
        ...
        (
          array_agg(
            ws.reps
            order by ws.weight_kg desc nulls last, ws.reps desc, ws.set_index
          )
        )[1] as reps_at_max
      from public.workout_exercises we
      join public.workout_sets ws on ws.workout_exercise_id = we.id
