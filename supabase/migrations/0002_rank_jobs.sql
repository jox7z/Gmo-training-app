...
begin
  ...
  -- 1. Aggregate previous week stats per user
  insert into public.weekly_stats (user_id, iso_year, iso_week, workout_days, total_volume_kg, goal_met)
  select
    w.user_id,
    prev_year,
    prev_week,
    count(distinct date_trunc('day', w.started_at)),
    coalesce(sum(w.total_volume_kg), 0),
