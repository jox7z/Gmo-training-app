...
export default function TodayScreen() {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    // Stores are local Zustand — re-render is automatic; small async delay
    // gives the pull-to-refresh a perceptible animation.
  ...
  const todayWorkouts = history.filter((w) => localDateKey(new Date(w.startedAt)) === todayKey);

  const handleStart = () => {
    if (!activeRoutine || !todayDay) return;
    const guard = canStartWorkout(history);
    if (!guard.allowed) {
      toast.show({ message: guard.reason ?? 'No puedes empezar ahora', tone: 'info' });
      return;
    }
    router.push({
    ...
    <Screen refreshing={refreshing} onRefresh={onRefresh}>
      ...
      <View style={{ marginTop: spacing['2xl'] }}>
        ...
        <View style={{ flexDirection: 'row', gap: spacing.md }}>
          ...
          <Card padding="lg" style={{ flex: 1 }}>
            <Stat label="Sets" value={weeklySets} unit="completados" tone="info" />
          </Card>
        </View>
      </View>
